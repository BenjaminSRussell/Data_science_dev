#!/usr/bin/env python3
"""
Specialized Map Asset Scraper
Scrapes Low-poly map elements (trees, buildings, roads, etc.)
"""
...
class MapAssetScraper:
    def __init__(self, output_dir="downloaded_assets/map/assets"):
        ...
        self.map_items = [
            'tree', 'building', 'house', 'road', 'path', 'grass', 'water',
            'rock', 'fence', 'gate', 'bench', 'streetlight', 'sign',
            'mailbox', 'lamp_post', 'fountain', 'statue', 'garden', 'hedge', 'wall'
        ]
    
    def resize_image(self, image_path, target_size=(128, 128)):
        """Resize map asset to target size"""
        try:
    ...
    def scrape_opengameart_map(self, item, max_results=20):
        """Scrape map assets from OpenGameArt"""
            ...
            try:
                ...
                for result in results[:max_results]:
                    ...
                    if download_link:
                        ...
                        filename = f"{item}_{len(self.downloaded)}.png"
    ...
    def run(self, target_count=500):
        """Run map asset scraping"""
        logger.info(f"Starting map asset scraping (target: {target_count})")
        
        for item in self.map_items:
            if self.stats['downloaded'] >= target_count:
                break
            
            logger.info(f"Scraping {item} assets...")
            count = self.scrape_opengameart_map(item, max_results=25)
            self.stats['downloaded'] += count
            self.stats['total'] += count
            logger.info(f"Downloaded {count} {item} assets, total: {self.stats['downloaded']}")
            time.sleep(2)
        
        self.save_manifest()
        logger.info(f"Map asset scraping complete: {self.stats['downloaded']} downloaded")
